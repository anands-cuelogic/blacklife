...
class CartridgeModel {
	getCartridge = () => {
		const query = "SELECT CartridgeCode , CartridgeName, SequenceNo, GroupName FROM Cartridge";
			...
			const temp = mySqlConnection.query(query, [ parameters ], (err, result) => {
				...
				if (err) {
					if (err.errno === 1062) {
						...
						return resolve({ success: true, data: result, requestedCombination: parameters });
					} else {
						// console.log("Error for createCartridgeCobination in cartridgeModel ", err);
						return reject({ success: false, message: err, requestedCombination: parameters });
					}
				}
				return resolve({ success: true, data: result, requestedCombination: parameters });
			});
	...
	getCartridgeCombinationByCode = (cartridgeCombinationCode) => {
		const query = "SELECT CartridgeCombinationCode FROM CartridgeCombination WHERE CartridgeCombinationCode IN (?)";
	...
	getCartridgeCombination = () => {
		const query = "SELECT CartridgeCombinationCode, CartridgeCombinationName FROM CartridgeCombination";
