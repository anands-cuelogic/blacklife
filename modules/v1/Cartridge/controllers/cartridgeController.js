...
class CartridgeController {
	...
	createStandardGas = async (standardGas) => {
		try {
			const standardGasCombinationResult = await this.standardGasCombination(standardGas);
	...
	createSingleGas = async (singleGas) => {
		try {
			const singleGasCartCombination = await this.singleGasCombination(singleGas);
